...
////////////////////////////////////////////////////////////////
 ...
 RCC->CFGR2 |= RCC_CFGR2_ADCPRE12_DIV2; //  ADC clock activate kar
 ...
 GPIOC->MODER |= 3 << (1*2); // adc ka channel 7 set kiya (on kiya)

 /* Calibration procedure but not required  */
 ADC1->CR &= ~ADC_CR_ADVREGEN;
 ADC1->CR |= ADC_CR_ADVREGEN_0; // 01: ADC Voltage regulator enabled

 ADC1->CR &= ~ADC_CR_ADCALDIF; //
 ADC1->CR |= ADC_CR_ADCAL; // mostly  ADC calibration start kiya
 ...
 // ADC configuration
  ADC1->CFGR |= ADC_CFGR_CONT; // ContinuousConvMode_Enable out of 5 mode of adc
  ADC1->CFGR &= ~ADC_CFGR_RES; // it set adc as 12-bit data resolution
  ADC1->CFGR &= ~ADC_CFGR_ALIGN;

 /* ADC1 regular channel7 configuration */
  ADC1->SQR1 |= ADC_SQR1_SQ1_2 | ADC_SQR1_SQ1_1 | ADC_SQR1_SQ1_0; // conversion starts hereeeeee
 ...
 while (1)
  {
  while(!(ADC1->ISR & ADC_ISR_EOC)); // not importabnt but test flag
  ADC1ConvertedValue = ADC1->DR; // Get ADC1 converted data
p=ADC1ConvertedValue/128;

ADC1ConvertedVoltage = (ADC1ConvertedValue *3300)/4096; // Compute the voltage == chutiyap
q=ADC1ConvertedVoltage/32;
